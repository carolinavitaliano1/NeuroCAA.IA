...
function openQuickEditModal(index) {
  ...
  modal.innerHTML = `
    ...
    <div style="background:#ecfdf5; padding:8px; border-radius:6px; margin-bottom:10px">
      💡 Clique na imagem para selecionar. Salve para aplicar.
    </div>
    ...
    <strong>Ou buscar outro pictograma</strong>
  ...
  async function loadImages(word) {
    ...
    try {
      ...
      grid.innerHTML = `<div style="grid-column:1/-1;color:red">Erro ao carregar</div>`;
    }
  }
  ...
  function renderImages(data) {
    ...
    data.slice(0, 9).forEach(p => {
      const src = `https://static.arasaac.org/pictograms/${p._id}/${p._id}_300.png`;
      ...
      img.style.cssText = `
        ...
        border-radius:8px;
        cursor:pointer;
        border:${tempImg === src ? '3px solid #22c55e' : '2px solid transparent'};
      `;

      img.onclick = (e) => {
        e.stopPropagation();     // 🔒 impede qualquer fechamento
        tempImg = src;           // apenas seleciona
